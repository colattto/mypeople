...
# ── ITEM 2 GATE: cross-navigation TODO <-> HUD (one connected system) ─────────
# The TODO page (:9933) must carry a link to the HUD (:9900/dashboard) and the HUD
# must carry a link to the TODO (:9933), so a human can jump between them.
curl -fsS http://127.0.0.1:9933/ | grep -q ':9900/dashboard' \
  || { echo "FAIL: TODO page has no cross-nav link to the HUD (:9900/dashboard)"; exit 1; }
curl -fsS http://127.0.0.1:9900/dashboard | grep -q ':9933' \
  || { echo "FAIL: HUD page has no cross-nav link to the TODO (:9933)"; exit 1; }
echo "cross-nav OK: TODO->HUD and HUD->TODO links both present"

# ── ITEM 3 GATE: clicking a commenter's agent name opens its terminal ─────────
# The card comment thread must wire an attachable commenter's name to the attach
# action, AND the /todo/attach resolver must turn an agent_id into a real ttyd
# target (mc-<sess>:<tab>) — the URL the click opens via the now-fixed ttyd attach.
# Prove BOTH the client wiring and the server-side resolution.
curl -fsS http://127.0.0.1:9933/ | grep -q 'ev-by.attach' \
  || { echo "FAIL: TODO comment authors are not click-to-terminal wired (ev-by.attach handler)"; exit 1; }
ATTACH_JSON=$(curl -fsS -H "X-Queue-Secret: $QSECRET" "http://127.0.0.1:9933/todo/attach?agent=$HOST_ID/main:Boss")
