...
| `TELEPROMPTER_SEED_URL` | no | Campaign link used in the system prompt. |

`## Verify` requires a real MyPeople runtime: `mp status` must work and the verify
will spawn or reuse a real MyPeople worker/agent. No fake worker, mocked brain, or
simulated MyPeople success is accepted. ManyChat/Instagram live transport is assumed
from the prior live prototype; verify captures the outbound `sendContent` payload locally
so the seed can prove the MyPeople brain path without a real ManyChat token.
...
SYSTEM = (
    ...
    "If the person asks for SEED/access, provide the configured seed URL. "
...
def _mp_reply(request, memory, timeout):
    ...
    prompt = (
        ...
        "then a colon, then the reply text. Do not copy the words 'reply text'."
    )
...
def ensure_real_worker():
    ...
    if agent not in status:
        ...
        sh(["mp", "spawn", short, "--backend", "codex", "--cwd", "/tmp"], timeout=180)
...
def main():
    ...
    work = Path(tempfile.mkdtemp(prefix="igra-verify-"))
